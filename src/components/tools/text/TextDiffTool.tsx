...
import { 
  Play, RotateCcw, Copy, Check, FileText, 
  Columns, AlignJustify 
} from 'lucide-react';

const SAMPLE_A = `Server Configuration:
Host: production-db-01.internal
Port: 5432
Max Connections: 100
SSL Mode: require
Timeout: 30s`;

const SAMPLE_B = `Server Configuration:
Host: production-db-02.internal
Port: 5432
Max Connections: 250
    ...
    <div className="space-y-6">
      <div className="backdrop-blur-xl bg-white/90 dark:bg-neutral-900/80 border border-black/10 dark:border-white/10 rounded-2xl p-5 shadow-xl transition-colors">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-4 mb-4 border-b border-black/10 dark:border-white/10 gap-3">
          <div>
            <div className="flex items-center gap-2 text-[11px] text-neutral-500 font-semibold mb-1">
              <a href="/" className="hover:underline">Home</a> <span>&gt;</span> <span className="text-neutral-900 dark:text-white">Text Diff</span>
            </div>
            <h1 className="text-lg font-bold text-neutral-900 dark:text-white flex items-center gap-2">
              <FileText size={18} /> Text Diff Inspector
            </h1>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-1.5">
            <div className="flex justify-between items-center text-[11px] font-mono">
              <span className="font-bold text-neutral-800 dark:text-neutral-200">Text A (Original)</span>
            </div>
            <textarea
              value={textA}
              onChange={(e) => setTextA(e.target.value)}
              placeholder="Paste original text here..."
              className="w-full h-56 p-3 font-mono text-xs bg-neutral-50 dark:bg-neutral-950 text-neutral-900 dark:text-neutral-100 border border-black/10 dark:border-white/15 rounded-xl focus:outline-none focus:ring-2 focus:ring-neutral-900 dark:focus:ring-white resize-y shadow-inner whitespace-pre-wrap"
            />
          </div>

          <div className="space-y-1.5">
            <div className="flex justify-between items-center text-[11px] font-mono">
              <span className="font-bold text-neutral-800 dark:text-neutral-200">Text B (Modified)</span>
            </div>
            <textarea
              value={textB}
        ...
        <div className="mt-4 flex justify-center">
          ...
          >
            <Play size={14} /> Compare Text Differences
          </button>
        </div>
      </div>
      ...
      {diffResult && (
        <div className="backdrop-blur-xl bg-white/90 dark:bg-neutral-900/80 border border-black/10 dark:border-white/10 rounded-2xl p-5 shadow-xl transition-colors">
          ...
          <div className="font-mono text-xs bg-neutral-50 dark:bg-neutral-950 text-neutral-900 dark:text-neutral-100 border border-black/5 dark:border-white/10 rounded-xl p-3 overflow-x-auto max-h-[500px]">
            {viewMode === 'unified' ? (
              <div className="space-y-0.5">
                {diffResult.map((part, index) => {
                  ...
                  return (
                    <div key={index} className={`${style} whitespace-pre-wrap break-all leading-relaxed`}>
              ...
              <div className="grid grid-cols-2 divide-x divide-black/10 dark:divide-white/10">
                <div className="pr-2 space-y-0.5">
                  ...
                  {buildSideBySideLines().map((row, index) => (
                    <div
                      key={index}
                      className={`whitespace-pre-wrap break-all leading-relaxed px-1 rounded-sm ${
